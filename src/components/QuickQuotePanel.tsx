...
export function QuickQuotePanel({ open, onClose }: QuickQuotePanelProps) {
  ...
  const inputStyle = (err: boolean): React.CSSProperties => ({
    ...
    border: `1px solid ${err ? '#e53935' : 'var(--border-color)'}`,
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ background: 'var(--red)', padding: '2rem 2rem 1.75rem', position: 'relative' }}>
          ...
          >×</button>
          <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.75)', marginBottom: '0.5rem', fontWeight: 600 }}>Project Enquiry — We respond within one business day</div>
        ...
        <div style={{ padding: '2rem', flex: 1 }}>
          {submitted ? (
            <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--fg-muted)', lineHeight: 1.7, marginBottom: '2rem' }}>Our team will review your request and respond within one business day.</p>
            ...
            <form onSubmit={handleSubmit} noValidate>
              <div style={{ marginBottom: '1.25rem' }}>
                <label style={{ display: 'block', fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600, color: errors.name ? '#e53935' : 'var(--fg-muted)', marginBottom: '0.5rem' }}>
                  Your Name {errors.name && <span style={{ color: '#e53935', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>— required</span>}
                </label>
                <input
                  value={form.name}
                  ...
                  onBlur={e => (e.target.style.borderColor = errors.name ? '#e53935' : 'var(--border-color)')}
                />
              </div>

              <div style={{ marginBottom: '1.25rem' }}>
                <label style={{ display: 'block', fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600, color: errors.phone ? '#e53935' : 'var(--fg-muted)', marginBottom: '0.5rem' }}>
                  Phone Number {errors.phone && <span style={{ color: '#e53935', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>— required</span>}
                </label>
                <input
                  value={form.phone}
                  ...
                  onFocus={e => (e.target.style.borderColor = 'var(--red)')}
                  onBlur={e => (e.target.style.borderColor = errors.phone ? '#e53935' : 'var(--border-color)')}
                />
              </div>

              <div style={{ marginBottom: '2rem' }}>
                <label style={{ display: 'block', fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600, color: errors.projectType ? '#e53935' : 'var(--fg-muted)', marginBottom: '0.5rem' }}>
                  Project Type {errors.projectType && <span style={{ color: '#e53935', fontWeight: 400, textTransform: 'none', letterSpacing: 0 }}>— required</span>}
                </label>
                  ...
                  {['Commercial', 'Residential', 'Industrial', 'Infrastructure', 'Engineering', 'Other'].map(t => (
                    <button
                      ...
                      style={{
                        ...
                        border: `1px solid ${form.projectType === t ? 'var(--red)' : errors.projectType ? '#e53935' : 'var(--border-color)'}`,
              ...
              <div style={{ display: 'flex', gap: 16, marginBottom: '1.5rem', padding: '1rem', background: 'var(--alt-bg)', borderLeft: '3px solid var(--red)' }}>
                {[{ val: '28 Years', sub: 'in Oman' }, { val: '20+ Projects', sub: 'delivered' }, { val: '1 Day', sub: 'response' }].map(item => (
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--fg-muted)', marginTop: '0.75rem', textAlign: 'center' }}>We respond within one business day.</p>
            </form>
          )}
        </div>
      </div>
