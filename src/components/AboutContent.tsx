    ...
    <main>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 'clamp(3rem,5vw,7rem)', alignItems: 'start' }} className="about-grid">
            ...
            <div ref={founderPhotoRef} style={{ position: 'sticky', top: '5.5rem' }}>
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: 'rgba(248,246,243,0.72)', lineHeight: 1.75, marginTop: '1.25rem', textAlign: isRTL ? 'right' : 'left' }}>
                {isRTL
                  ? 'أسّس شركة البناء عام 1997 تحت مظلة مجموعة الخنجي، ثلاثون عاماً من الملكية المباشرة والإشراف الشخصي على كل مشروع.'
                  : 'Founded Al Binaa in 1997 under the Al Khonji Group, thirty years of direct ownership and hands-on oversight of every project.'}
              </p>
            </div>
              ...
              <blockquote style={{ margin: 0, padding: 0 }}>
                ...
                <footer>
                  ...
                  <div style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'rgba(248,246,243,0.65)', marginTop: 4, letterSpacing: isRTL ? '0' : '0.08em', textTransform: isRTL ? 'none' : 'uppercase' }}>
                    {isRTL ? 'رئيس مجلس الإدارة، البناء للمقاولات والصناعة ش.م.ع.م' : 'Chairman, Al Binaa Construction & Industry SAOC'}
                  </div>
                </footer>
              </blockquote>
            </div>
      ...
      <section style={{ background: 'var(--section-bg)', padding: 'clamp(4rem,8vw,8rem) clamp(1.5rem,5vw,4rem)' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto' }}>
          ...
          {/* Vision & Mission, prominent editorial moment */}
          <div ref={manifestoRef} style={{ borderTop: '1px solid var(--border-color)', paddingTop: 'clamp(2.5rem,4vw,4rem)' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.4rem,5vw,4rem)', fontWeight: 700, color: 'var(--fg)', margin: '0 0 clamp(2rem,3.5vw,3rem)', lineHeight: 1.02, letterSpacing: '-0.01em' }}>
              {isRTL ? 'الرؤية والرسالة' : 'Vision & Mission'}
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(2.5rem,5vw,5rem)', alignItems: 'start' }} className="about-grid">
              <div style={{ borderInlineStart: '3px solid var(--red)', paddingInlineStart: 'clamp(1.25rem,2vw,1.75rem)' }}>
                <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: isRTL ? '0' : '0.16em', textTransform: isRTL ? 'none' : 'uppercase', fontWeight: 700, color: 'var(--red)', marginBottom: '1rem' }}>
                  {isRTL ? 'رؤيتنا' : 'Our Vision'}
                </div>
                <p style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.15rem,1.9vw,1.5rem)', fontWeight: 600, color: 'var(--fg)', lineHeight: 1.5, margin: 0, letterSpacing: '-0.01em' }}>
                  {isRTL
                    ? 'أن نكون المقاول الذي يعود إليه كبار المطورين والعملاء المؤسسيين في عُمان، ليس بسبب التسويق، بل بفضل ثلاثين عاماً من التسليم المتواصل.'
                    : 'To be the contractor Oman’s most serious developers and institutional clients return to, not because of marketing, but because of thirty years of uninterrupted delivery.'}
                </p>
              </div>
              <div style={{ borderInlineStart: '3px solid var(--red)', paddingInlineStart: 'clamp(1.25rem,2vw,1.75rem)' }}>
                <div style={{ fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: isRTL ? '0' : '0.16em', textTransform: isRTL ? 'none' : 'uppercase', fontWeight: 700, color: 'var(--red)', marginBottom: '1rem' }}>
      ...
      <section style={{ background: 'var(--red)', padding: 'clamp(2.5rem,4vw,4rem) clamp(1.5rem,5vw,4rem)' }}>
        <div style={{ maxWidth: 1400, margin: '0 auto' }}>
          <div ref={numbersRef} style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)' }} className="stats-grid">
      ...
      {/* ── 3. WHY AL BINAA — concise pointer to the dedicated page ──────────────── */}
      <section style={{ background: 'var(--alt-bg)', padding: 'clamp(3rem,6vw,5rem) clamp(1.5rem,5vw,4rem)' }}>
        <div ref={whyHeadRef} style={{ maxWidth: 1400, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '2rem', flexDirection: isRTL ? 'row-reverse' : 'row' }}>
          <div style={{ maxWidth: 640 }}>
            <SectionLabel>{isRTL ? 'لماذا البناء' : 'Why Al Binaa'}</SectionLabel>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.7rem,3vw,2.4rem)', fontWeight: 700, color: 'var(--fg)', margin: '0 0 0.75rem', lineHeight: 1.1 }}>
            ...
            <p style={{ fontFamily: 'var(--font-body)', fontSize: 14.5, color: 'var(--fg-muted)', lineHeight: 1.8, margin: 0 }}>
              {isRTL ? 'عقد واحد، اعتماد ISO، ودعم مجموعة الخنجي. ثلاثون عاماً من الأدلة لا الوعود.' : 'One contract, ISO certification, and Al Khonji Group backing. Thirty years of evidence, not promises.'}
            </p>
          </div>
          <Link href={`/${locale}/why-al-binaa`} className="btn-press"
            style={{ background: 'var(--red)', color: '#fff', textDecoration: 'none', fontFamily: 'var(--font-body)', fontSize: 11, letterSpacing: isRTL ? '0' : '0.14em', textTransform: isRTL ? 'none' : 'uppercase', fontWeight: 600, padding: '14px 32px', whiteSpace: 'nowrap', transition: 'opacity 0.2s', flexShrink: 0 }}
            onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.opacity = '0.85'; }}
            onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.opacity = '1'; }}>
            {isRTL ? 'لماذا البناء ←' : 'See Why Al Binaa →'}
          </Link>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1400, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center', marginBottom: 'clamp(3rem,5vw,5rem)' }} className="about-grid">
            <div ref={teamRef}>
              ...
              <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2rem,3.5vw,2.8rem)', fontWeight: 700, color: 'var(--fg)', margin: '0 0 1.25rem', lineHeight: 1.05 }}>
                {isRTL ? 'أكثر من 1,000+ متخصص' : '1,000+ Directly Employed.'}
              </h2>
